...
async function initTmap() {
    ...
    $("#btn_select").click(async function () {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            for (var k in resultpoisData) {
                                ...
                                listItem.text(name);
                                listItem.attr('data-poi-id', poiId); // 데이터 속성으로 poiId 추가
                                resultList.append(listItem);
                            }

                            // 생성된 리스트의 각 항목에 클릭 이벤트 추가
                            $(document).on('click', '.result-item', function () {
                                ...
                                $.ajax({
                                    method: "GET",
                                    headers: headers,
                                    url: "https://apis.openapi.sk.com/tmap/pois/" + poiId + "?version=1&format=json&callback=result",
                                    success: function (response) {
                                        var poiData = response.searchPoiInfo.pois.poi;
                                        var lon = Number(poiData.noorLon);
                                        var lat = Number(poiData.noorLat);
                                        ...
                                        showPOIDetailModal(poiId); // 클릭된 항목의 상세 정보 표시
                                    }
                                });
                            });
                        },
...
function showPOIDetailModal(poiId) {
    var headers = {};
    headers["appKey"] = "pRNUlsEpce4d3mB0MUabnMDhHbLmdtlPrUYZI3i0";

    // POI 상세 정보를 가져오는 API 호출
    $.ajax({
        method: "GET",
        headers: headers,
        url: "https://apis.openapi.sk.com/tmap/pois/" + poiId + "?version=1&format=json&callback=result",
        success: function (response) {
            var poiData = response.searchPoiInfo.pois.poi;
            var name = poiData.name;
            var address = poiData.upperAddrName + " " + poiData.middleAddrName + " " + poiData.lowerAddrName;

            // 모달에 포이 디테일 정보 채우기
            var modalContent = '<div class="modal-body">';
            modalContent += '<div class="poi-name">' + name + '</div>';
            modalContent += '<div class="poi-address">' + address + '</div>';
            modalContent += '</div>';

            showModal(modalContent); // 모달 보이기
        },
        error: function (request, status, error) {
            console.log("code:" + request.status + "\n" + "message:" + request.responseText + "\n" + "error:" + error);
        }
...
function showModal(content) {
    // 모달 창을 찾아서 jQuery로 선택합니다.
    var modal = $("#poiDetailModal");
